...
public class PossibleMoves {
     ...
     *               ArrayLists of Tile
     *               objects.
     * @param gBoard The remainingSeq parameter is an ArrayList of ArrayLists of
     *               Tile objects. It
     *               represents the remaining sequences of tiles that can be added
     *               to the current board state. Each
     *               inner ArrayList represents a sequence of tiles.
     * @param b      gBoard is an ArrayList of ArrayLists of ArrayLists of Tile
     *               objects. It represents
     *               the game board state, where each ArrayList<Tile> represents a
     *               row on the board, and each
     *               ArrayList<ArrayList<Tile>> represents the entire board.
     * @param hand   The parameter "b" is of type Board.
     */
    public static void makeBoardState(ArrayList<ArrayList<Tile>> cBoard, ArrayList<ArrayList<ArrayList<Tile>>> gBoard,
            ArrayList<ArrayList<Tile>> gHand, ArrayList<Tile> boardandhand, ArrayList<Tile> hand,
            ArrayList<Tile> unusedTiles) {
        ArrayList<ArrayList<Tile>> gLines;
        if (boardandhand.isEmpty()) {
            gLines = new ArrayList<>();
        } else
            gLines = getLines(boardandhand);
        // We cannot create any new lines. We check if we used all the board and at
        // least one from hand - if so than it is a valid board
        // And if we have enough tiles left, we skip current first as unused and try to
        // make rest of the board without it
        if (gLines.isEmpty()) {
    ...
    public static ArrayList<Object[]> possibleMoves(Board b, ArrayList<Tile> hand, int makeBoardType) {
        ...
        // System.out.println(hand.size() + boa.size() + " are all the tiles we try to
        // combine to moves");
